...
module pahm_mod
  ...
  !use GLOBAL , only: ETA2, UU2, VV2  ! Export water level and velocity fileds to pahm model
  !USE GLOBAL,  ONLY: RSNX2, RSNY2    ! Import pahm 2D forces from pahm model
  ...
  implicit none
    ...
    ! info for reading pahm netcdf file
...
!-----------------------------------------------------------------------
    SUBROUTINE init_pahm_nc()
      ...
      character(len=*),parameter :: subname='(pahm_mod:init_pahm_nc)'
      ...
      write(info,*) subname,' --- init pahm netcdf file  --- '
      !print *, info
...
!- Sub !!!????
!-----------------------------------------------------------------------
    SUBROUTINE read_pahm_nc(currTime)
      IMPLICIT NONE
      ...
      character (len = 140)          :: units
      character(len=*),parameter :: subname='(pahm_mod:read_pahm_nc)'
      ...
      if (abs(delta_d_all (it)) .gt. 7200.) then
         write(info,*) subname,' --- STOP PAHM: Time dif is GT 2 hours ---  '
         call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=rc)
      ...
      write(info,*) subname,'pahm file it index > ',it
      !print *, info
      call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=rc)
      ...
      !write(info,*) ' Read PAHM netcdf:',it, 
      !call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=rc)
      ...
      write(info,*) subname,' --- read PAHM netcdf file  --- '
      !print *, info
      call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=rc)
    ...
    subroutine read_config()
     ...
     !   return  ! bail out
      ...
      call ESMF_ConfigGetAttribute(cf, atm_nam, label="atm_nam:", &
           default='pahm.Constant.YYYYMMDD_sxy.nc'  , rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
