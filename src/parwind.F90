...
!    !----- converted data from the above values (if needed)
  ...
  SUBROUTINE ReadBestTrackFile()
    ...
    CHARACTER(LEN=512)             :: inpLine, line
    CHARACTER(LEN=512)             :: fmtStr

    INTEGER                        :: lenLine
    INTEGER                        :: nLines                  ! Number of lines counter
    ...
    !fmtStr = '(a2, 2x, i2, 2x, a10, 2x, i2, 2x, a4, 2x, i3, 2x, i3, a1, 2x, i4, a1, 2x, i3, 2x, i4, 2x, a2, '
    fmtStr = '(a2, 1x, i3, 2x, a10, 1x, i3, 2x, a4, 2x, i3, 2x, i3, a1, 1x, i5, a1, 2x, i3, 2x, i4, 2x, a2, '
      fmtStr = TRIM(fmtStr) // ' 2x, i3, 2x, a3, 4(2x, i4), 2x, i4, 2x, i4, 2x, i3, 2x, i3, 2x, i3, '
      fmtStr = TRIM(fmtStr) // ' 2x, a3, 2x, i3, 2x, a3, 1x, 2(i3, 2x), a11)'
    !----------

    CALL SetMessageSource("ReadBestTrackFile")
    ...
    DO iFile = 1, nBTrFiles
      ...
      CALL OpenFileForRead(iUnit, TRIM(ADJUSTL(inpFile)), errIO)

      IF (errIO /= 0) THEN
        WRITE(scratchMessage, '(a)') 'Error opening the best track file: ' // TRIM(ADJUSTL(inpFile))
        ...
        CALL Terminate()
      ELSE
        WRITE(scratchMessage, '(a)') 'Processing the best track file: ' // TRIM(ADJUSTL(inpFile))
      ...
      ! Count the number of non-empty or commented out lines in the file.
      ! Comments are are considered those lines with the first non-blank character of "!" or "#"
      nLines = 0
      DO
        READ(UNIT=iUnit, FMT='(a)', ERR=10, END=5, IOSTAT=errIO) inpLine

        lenLine = GetLineRecord(inpLine, line)
        IF (lenLine /= 0) nLines = nLines + 1
      END DO
      5 REWIND(UNIT=iUnit)
      ...
      kCnt = 0
      DO WHILE (.TRUE.)
        READ(UNIT=iUnit, FMT='(a)', ERR=10, END=20, IOSTAT=errIO) inpLine

        lenLine = GetLineRecord(inpLine, line)

        IF (lenLine /= 0) THEN
          iCnt = iCnt + 1
          READ(line, FMT=fmtStr, ERR=11, IOSTAT=ios)                                     &
            bestTrackData(iFile)%basin(iCnt),     bestTrackData(iFile)%cyNum(iCnt),      &
            bestTrackData(iFile)%dtg(iCnt),       bestTrackData(iFile)%techNum(iCnt),    &
            bestTrackData(iFile)%tech(iCnt),      bestTrackData(iFile)%tau(iCnt),        &
            bestTrackData(iFile)%intLat(iCnt),    bestTrackData(iFile)%ns(iCnt),         &
            bestTrackData(iFile)%intLon(iCnt),    bestTrackData(iFile)%ew(iCnt),         &
            bestTrackData(iFile)%intVMax(iCnt),   bestTrackData(iFile)%intMslp(iCnt),    &
            bestTrackData(iFile)%ty(iCnt),        bestTrackData(iFile)%rad(iCnt),        &
            bestTrackData(iFile)%windCode(iCnt),  bestTrackData(iFile)%intRad1(iCnt),    &
            bestTrackData(iFile)%intRad2(iCnt),   bestTrackData(iFile)%intRad3(iCnt),    &
            bestTrackData(iFile)%intRad4(iCnt),   bestTrackData(iFile)%intPOuter(iCnt),  &
            bestTrackData(iFile)%intROuter(iCnt), bestTrackData(iFile)%intRmw(iCnt),     &
            bestTrackData(iFile)%gusts(iCnt),     bestTrackData(iFile)%eye(iCnt),        &
            bestTrackData(iFile)%subregion(iCnt), bestTrackData(iFile)%maxseas(iCnt),    &
            bestTrackData(iFile)%initials(iCnt),  bestTrackData(iFile)%dir(iCnt),        &
            bestTrackData(iFile)%intSpeed(iCnt),  bestTrackData(iFile)%stormName(iCnt)
      ...
      10 IF (errIO /= 0) THEN
           WRITE(scratchMessage, '(a)') 'Error in file: '  // TRIM(ADJUSTL(inpFile)) // &
                                        ', while processing line: ' // TRIM(ADJUSTL(inpLine))
           CALL AllMessage(ERROR, scratchMessage)
      ...
      11 IF (ios /= 0) THEN
           WRITE(scratchMessage, '(a)') 'Error in file: '  // TRIM(ADJUSTL(inpFile)) // &
                                        ', while processing line: ' // TRIM(ADJUSTL(line))
           CALL AllMessage(ERROR, scratchMessage)
      ...
      DO iCnt = 1, bestTrackData(iFile)%numRec
        castType = ToUpperCase(TRIM(ADJUSTL(bestTrackData(iFile)%tech(iCnt))))
        IF (iCnt == 1) THEN
          kCnt = iCnt
          bestTrackData(iFile)%cycleNum(iCnt) = kCnt
        ELSE
          IF (bestTrackData(iFile)%dtg(iCnt) /= &
              bestTrackData(iFile)%dtg(iCnt-1) .AND. castType /= 'BEST') kCnt = kCnt + 1
        END IF
...
!================================================================================
  ...
  SUBROUTINE ProcessAsymmetricVortexData(idTrFile, strOut, status)
    ...
    USE Utilities, ONLY     : ToUpperCase, CharUnique, SphericalDistance
    USE TimeDateUtils, ONLY : TimeConv
    USE PaHM_Vortex
...
!PV DO WE NEED TO INCLUDE THE SAME CODE FOR CASTTIME AS IN HOLLAND?
      ...
      !PV DO we need cycleTime or CastTime instead?
      IF (iCnt /= 1) THEN
...
!================================================================================
  ...
  SUBROUTINE GetGAHMFields(timeIDX)
    ...
    USE PaHM_Global, ONLY   : backgroundAtmPress, ONE2TEN,                                              &
                              ...
                              Times, DatesTimes,                                                        &
                              wVelX, wVelY, wPress
    USE Utilities, ONLY     : ToUpperCase, SphericalDistance, SphericalFracPoint, GetLocAndRatio
    USE TimeDateUtils, ONLY : JulDayToGreg, GregToJulDay, GetTimeConvSec, DateTime2String
    ...
    INTEGER,  DIMENSION(:),    ALLOCATABLE, SAVE            :: nCycles, iCycTrk
    ...
    REAL(SZ), SAVE                      :: numRmaxes ! number of Rmax values selected by user
      ...
      DO stCnt = 1, nBTrFiles ! HERE
        ...
        DO kCnt = 1, asyVortStru(stCnt)%numRec
         ...
         IF (kCnt == 1) THEN
           ...
           ! We use timeOffset to relate the cycleTime to the simulation time. If
           ! asyVortStru(stCnt)%fcstInc(kCnt) = 0, then timeOffset = 0.0
           timeOffset =  asyVortStru(stCnt)%fcstInc(kCnt) *3600.0_SZ

           cycleTime(stCnt, iCyc)        = mdBegSimTime - timeOffset + asyVortStru(stCnt)%fcstInc(kCnt) * 3600.0_SZ
           !cycleTime(stCnt, iCyc)        = asyVortStru(stCnt)%fcstInc(kCnt) * 3600.0_SZ
         ...
         ELSE ! kCnt == 1
           ...
           END DO
         END IF ! kCnt /= 1
        END DO ! asyVortStru(stCnt)%numRec

        nCycles(stCnt) = asyVortStru(stCnt)%nCycles
        iCycTrk(stCnt) = 1 ! Is this realy ok? PV CHECK THIS PLEASE
      END DO ! HERE
...
!################################################################
    ...
    DO stCnt = 1, nBTrFiles
      ...
      CALL GetLocAndRatio(Times(iCnt), cycleTime(stCnt, :), jl1, jl2, wtRatio)
      ...
      ! Arithmetic mean of the Rmax in the selected storm quadrants from
      ! the selected isotachs
      numRMaxes = 0.0_SZ ! count how many Rmax values we have to work with
      DO i = 1, 4   ! loop quads
        DO j = 1, 3 ! loop isotachs
          IF (quadFlag(stCnt, jl1, i, j) == 1) THEN
             oldVortexRMW = oldVortexRMW + rmaxw(stCnt, jl1, i, j)
             numRMaxes = numRMaxes + 1.0_SZ
          END IF
        END DO
      END DO
      ...
      DO i = 1, 4  ! loop quads
        DO j = 1, 3  ! loop isotachs
          IF (quadFlag(stCnt, jl2, i, j) == 1) THEN
            newVortexRMW = newVortexRMW + rmaxw(stCnt, jl2, i, j)
            numRMaxes = numRMaxes + 1.0_SZ
          END IF
        END DO
      END DO
